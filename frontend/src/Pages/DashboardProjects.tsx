...
export default function DashboardProjects() {
	const fetchMyProjects = useUserProjectStore(
		(state) => state.fetchMyProjects
	);
	...
	const frontendProjects =
		userProjects?.filter((p) => p.tag === "Frontend").length || 0;
	const backendProjects =
		userProjects?.filter((p) => p.tag === "Backend").length || 0;
	const fullStackProjects =
		userProjects?.filter((p) => p.tag === "Fullstack").length || 0;
							...
							{stats.map((stat, index) => (
								<motion.div
									...
									<div className='flex justify-between items-center'>
										...
										<div
											className={`flex items-center justify-center w-10 h-10 bg-gradient-to-br ${stat.color} rounded-lg shadow-sm`}>
